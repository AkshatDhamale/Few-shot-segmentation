...
def parse_arguments():
    ...
    parser.add_argument('--data_root', type=str, default='results_abd/test')
...
def main():
    ...
    for label_val, label_name in labels.items():
        ...
        # Infer.
        with torch.no_grad():
            scores = infer(model, query_loader, support_sample, args, logger, label_name, quarters)
    ...
    logger.info('  *--------------------------------------------------*')
    logger.info('  First Quarter: {}'.format(quarters[0]))
    logger.info('  Second Quarter: {}'.format(quarters[1]))
    logger.info('  Third Quarter: {}'.format(quarters[2]))
    logger.info('  Fourth Quarter: {}'.format(quarters[3]))
    logger.info('  *--------------------------------------------------*')


def infer(model, query_loader, support_sample, args, logger, label_name, quarters):
    ...
    for i, sample in enumerate(query_loader):
        ...
        if args.EP1 is True:
            ...
            testimg = sitk.GetImageFromArray(query_pred)
            sitk.WriteImage(testimg, 'image_' + str(i) + '.nii.gz')

        # Record scores.
        scores.record(query_pred, query_label)

        dice_score = str(scores.patient_dice[-1].item())       

        if (dice_score <= '0.25'):
            quarters[0] += 1
        elif (dice_score > '0.25' and dice_score <= '0.50'):
            quarters[1] += 1
        elif (dice_score > '0.50' and dice_score <= '0.75'):
            quarters[2] += 1
        else:
            quarters[3] += 1
        ...
        # Save predictions.
        file_name = 'image_' + str(i) + '_' + str(i) + '.pt'
