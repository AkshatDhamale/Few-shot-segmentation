...
class TestDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        self.image_dirs = self.image_dirs[:len(self.image_dirs)//5]
    ...
    def __getitem__(self, idx):
        ...
        # Evaluation protocol 2 (default).
            ...
            # sample['label'] = torch.from_numpy(full_brain_slices)
    ...
    def getSupport(self, label=None, all_slices=True, N=None):
        ...
        if all_slices:
            ...
            # sample['label'] = torch.from_numpy(full_brain_slices[idx][idx_])
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        self.label_dirs = glob.glob(os.path.join(args.data_root, 'BraTS_T2_normalized/label*'))
        # self.label_dirs = glob.glob(os.path.join(args.data_root, 'supervoxels_' + str(args.n_sv), 'super*'))
        self.label_dirs = sorted(self.label_dirs, key=lambda x: int(x.split('_')[-1].split('.nii.gz')[0]))
        self.label_dirs = self.label_dirs[:len(self.label_dirs)//20]
        ...
        if self.read:
            self.images = {}
            self.sprvxls = {}
            for image_dir, sprvxl_dir in zip(self.image_dirs, self.label_dirs):
    ...
    def __getitem__(self, idx):
        ...
        if self.read:
            ...
            sprvxl = self.sprvxls[self.label_dirs[pat_idx]]
        else:
            # read image/supervoxel volume into memory
            img = sitk.GetArrayFromImage(sitk.ReadImage(self.image_dirs[pat_idx]))
            sprvxl = sitk.GetArrayFromImage(sitk.ReadImage(self.label_dirs[pat_idx]))
